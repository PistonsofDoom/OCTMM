...
| ADSR | Attack, Decay, Sustain, Release. If input is positive, the ADSR starts. Output is always between 0 and 1. Any empty or nil parameters are set to 0. | <ul><li>Attack in Beats</li><li>Decay in Beats</li><li>Sustain level (0.0 -> 1.0)</li><li>Release in Beats</li></ul> | <ol><li>ADSR Control</li></ol> |
...
#### ADSR
```lua
local sine_network = Constant.new(440.0) .. Sine

local adsr_network = Constant.new(1.0) .. ADSR(0.25, 1)
local sine_adsr_network = sine_network * adsr_network
```
...
local clipped_network = (sine_network * 2.0) .. Clip()
```
